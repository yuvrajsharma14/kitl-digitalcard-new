...
export function TemplateForm({ mode, templateId, defaultValues }: TemplateFormProps) {
  ...
  const {
    ...
    defaultValues: {
      ...
      config: defaultValues?.config ?? DEFAULT_TEMPLATE_CONFIG,
    },
  });

  const config = watch("config") as TemplateConfig;
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        <section className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm space-y-5">
          ...
          <div className="space-y-2">
            ...
            <Input
              id="name"
              placeholder="e.g. Professional Dark"
              {...register("name")}
            />
            {errors.name && (
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">
                Visible to users in the card builder
              </p>
            </div>
            <Controller
              name="isActive"
